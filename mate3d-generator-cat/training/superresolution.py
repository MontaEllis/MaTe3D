...
from training.networks_stylegan2 import SynthesisBlock, SynthesisBlock_Dual
import numpy as np
...
class SPADE(torch.nn.Module):
    def __init__(self, input_dim,origin_dim, nhidden,fp16):
        # super().__init__()
 
        super(SPADE, self).__init__()
        self.channels_last = fp16
        self.param_free_norm = torch.nn.InstanceNorm2d(nhidden, affine=False)
        # self.param_free_norm_1 = nn.InstanceNorm2d(nhidden, affine=False)
        # The dimension of the intermediate embedding space. Yes, hardcoded.
        # nhidden = 256
        ks = 5
        pw = ks // 2
        self.mlp_shared = torch.nn.Sequential(
            Conv2dLayer(input_dim, nhidden, kernel_size=ks,channels_last=self.channels_last),
            # torch.nn.Conv2d(input_dim, nhidden, kernel_size=ks, padding=pw),
            torch.nn.ReLU()
        )
 
        # self.param_free_norm = nn.Sequential(
        #     nn.Conv2d(origin_dim, nhidden, kernel_size=ks, padding=pw),
        #     nn.ReLU()
        # )
 
        # self.mlp_gamma = torch.nn.Conv2d(nhidden, nhidden, kernel_size=ks, padding=pw)
        self.mlp_gamma = Conv2dLayer(nhidden, nhidden, kernel_size=ks, channels_last=self.channels_last)
        # self.mlp_beta = torch.nn.Conv2d(nhidden, nhidden, kernel_size=ks, padding=pw)
        self.mlp_beta = Conv2dLayer(nhidden, nhidden, kernel_size=ks, channels_last=self.channels_last)
    ...
    def forward(self, x, segmap):
        ...
        # normalized = F.interpolate(normalized, size=segmap.size()[2:], mode='nearest')
        # segmap = segmap.unsqueeze(1)#.repeat(1,1,1,1)
        # segmap = self.param_free_norm_1(segmap)
        segmap = F.interpolate(segmap, size=normalized.size()[2:], mode='nearest')
