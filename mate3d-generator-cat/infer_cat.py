...
@click.command()
@click.option('--network', 'network_pkl', help='Network pickle filename', required=False, default='/openbayes/input/input0/mate3d-mask-cat.pkl')
...
def generate_images(
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for angle_y, angle_p in [(20/180*np.pi, angle_p),(10/180*np.pi, angle_p), (0, angle_p), (-10/180*np.pi, angle_p),(-20/180*np.pi, angle_p)]:
            ...
            seg_raw = (torch.argmax(seg_raw,1).unsqueeze(1).permute(0, 2, 3, 1) /6*255).clamp(0, 255).to(torch.uint8)
            seg = (torch.argmax(seg,1).unsqueeze(1).permute(0, 2, 3, 1) /6*255).clamp(0, 255).to(torch.uint8)
